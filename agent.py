...
class Agent:
    ...
    def _create_pydantic_agent(self, config: AgentConfig) -> PydanticAgent:
        ...
        model_settings = config.settings if config.settings else OpenAIResponsesModelSettings(openai_reasoning_effort='minimal')
        ...
        if config.tools:
            agent_kwargs["tools"] = list(config.tools)
    ...
    async def run(self, input_payload: Optional[Dict[str, Any]] = None) -> Any:
        """Execute without persistence (async); returns pydantic_ai result."""
        import json
    ...
    def run_sync(self, input_payload: Optional[Dict[str, Any]] = None) -> Any:
        """Execute without persistence; returns pydantic_ai result."""
        import json
