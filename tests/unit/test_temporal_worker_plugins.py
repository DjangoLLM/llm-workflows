...
from agents.temporal.worker_plugins import (
...
def test_build_plugin_module_paths_rejects_duplicates(settings) -> None:
    settings.AGENTS_TEMPORAL_PLUGIN_MODULES = ["agents.temporal.plugins.core"]
...
def test_load_plugin_rejects_missing_entrypoint(monkeypatch) -> None:
    monkeypatch.setattr(
        "agents.temporal.worker_plugins.importlib.import_module",
...
def test_build_temporal_worker_composition_loads_core_and_custom(settings, monkeypatch) -> None:
    ...
    modules = {
        "agents.temporal.plugins.core": SimpleNamespace(get_temporal_worker_plugin=lambda: core_plugin),
    ...
    assert composition.plugin_modules == ["agents.temporal.plugins.core", "custom.plugin"]
