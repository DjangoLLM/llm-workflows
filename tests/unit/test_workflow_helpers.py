...
from agents.temporal.workflows import WorkflowExecutionHelpers
...
def test_fail_workflow_executes_failure_activity(monkeypatch) -> None:
    ...
    monkeypatch.setattr("agents.temporal.workflows.workflow.logger", DummyLogger())
    monkeypatch.setattr("agents.temporal.workflows.workflow.execute_activity", fake_execute_activity)
