...
def test_register_all_steps_rejects_non_callable_entry(settings, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agents.core.step_registrations.import_string", lambda _path: object())
...
def test_register_all_steps_is_idempotent(settings, monkeypatch) -> None:
    ...
    monkeypatch.setattr("agents.core.step_registrations.import_string", lambda _path: registrar)
