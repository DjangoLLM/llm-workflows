...
def decide(*, instructions: str, questions: Mapping[str, Mapping[str, Any]], model: str | None, state: Any) -> dict:
    ...
    if not key:
        raise RuntimeError("TYPESAFE_API_KEY is not set; Jev evaluation cannot run.")
...
class JevRunner(ChoiceAdapter[SimpleNamespace]):
    ...
    def __post_init__(self) -> None:
        if not self.questions or any(
            ...
            raise ValueError(
                "JevRunner requires `questions` with a `criteria` mapping per head"
            )
        if self.model is not None and not isinstance(self.model, str):
            raise ValueError("JevRunner takes `model` as a string, e.g. 'jev-latest'")
