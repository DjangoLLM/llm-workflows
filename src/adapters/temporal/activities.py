...
from agents.runner.choice import JevIf
...
def create_pipeline_run_activity(pipeline_name: str, payload: dict) -> str:
    ...
    run_id = create_workflow_run(pipeline_name, payload)
...
def execute_pipeline_step_activity(
    ...
    registration = StepCatalog.get_step(step_key)
    if registration.pipeline_name != pipeline_name:
        raise ValueError(
            f"Step '{step_key}' belongs to '{registration.pipeline_name}', "
            f"not '{pipeline_name}'."
        )
    return execute_workflow_step(
        ...
        payload=payload,
        agent_definition=StepDispatcher.resolve_agent_definition(registration),
    )
...
def mark_pipeline_success_activity(run_id: str, pipeline_name: str) -> None:
    ...
    mark_workflow_success(run_id, pipeline_name)
...
def mark_pipeline_failed_activity(run_id: str, pipeline_name: str, error_message: str) -> None:
    ...
    mark_workflow_failure(run_id, pipeline_name, error_message)
